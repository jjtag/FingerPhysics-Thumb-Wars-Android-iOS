...
static NSString *GTMStackTraceFromAddressDescriptors(struct GTMAddressDescriptor descs[],
	...
	for (NSUInteger i = 0; i < count; i++) {
		...
		if (descs[i].class_name) {
			[trace appendFormat:@"%s[%s %s]" ,(descs[i].is_class_method ? "+" : "-"),
			 descs[i].class_name,
			 (descs[i].symbol ? descs[i].symbol : "??")];
		} else {
			[trace appendFormat:@"%s()", (descs[i].symbol ? descs[i].symbol : "??")];
		}
...
NSString *GTMStackTrace(int skipFrames) {
	...
	if (count) {
		...
		if (pcs && descs) {
			...
			// start w/ this frame.
			const size_t kTracesToStrip = skipFrames;
...
NSString *GTMStackTraceFromException(NSException *e, int skipFrames) {
	NSString *trace = @"";
	...
	if (count) {
		...
		if (pcs && descs) {
			...
			const size_t kTracesToStrip = skipFrames;
			if (count > kTracesToStrip) {
				trace = GTMStackTraceFromAddressDescriptors(&descs[kTracesToStrip],
															 (count - kTracesToStrip));
			}
		}
		if (pcs) free(pcs);
