...
IID NSData::initWithData(NSData *data)
{
    init();
    ...
    data->getByteslength(bytes, length);

    return this;
}

IID NSData::initWithBytes(void* data, int len)
{
    init();
...
IID NSData::initWithContentsOfFile(NSString *path)
{
    if (!init())
        return nil;
    ...
    for (int i = 0; i < len; i++)
        chars[i] = (jchar)path->value[i];
    jstring jpath = jenv->NewString(chars, len);
    delete[] chars;

    jclass c = jenv->GetObjectClass(loader);
    jmethodID m = jenv->GetMethodID(c, "loadData", "(Ljava/lang/String;)[B");
    jbyteArray data = (jbyteArray)jenv->CallObjectMethod(loader, m, jpath);

    jenv->DeleteLocalRef(jpath);
    jenv->DeleteLocalRef(c);
    length = jenv->GetArrayLength(data);
    bytes = new char[length];
    jenv->GetByteArrayRegion(data, 0, length, (jbyte*)bytes);
    jenv->DeleteLocalRef(data);

    return this;
}

IID NSData::dataWithData(NSData *data)
{
    return NSData::alloc()->initWithData(data)->autorelease();
}

IID NSData::dataWithContentsOfFile(NSString *path)
{
    return NSData::alloc()->initWithContentsOfFile(path)->autorelease();
}

BOOL NSData::isEqualToData(NSData *data)
{
    return true;
}

void NSData::getBytesrange(void* result, NSRange range)
{
    char* to = (char*)result;
    char* from = bytes + range.location;
...
void NSData::getByteslength(void* result, NSUInteger length)
{
    NSRange range = { 0, length };
    getBytesrange(result, range);
}

void NSData::getBytes(void* result)
{
    getByteslength(result, length);
}
